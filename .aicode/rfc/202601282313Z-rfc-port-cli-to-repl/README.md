...
### Rework summary
- Relocated all package code into top-level `src/`, `tests/`, and `resources/`, removing the nested package layout.
- Refactored `repl.py` into `src/repl.py` with a focused pipeline that delegates XLSX sampling, CSV/DOCX matching, and parquet enrichment into dedicated modules.
- Implemented sampler logic from `sampler.py` and `pilot_sampler.py`, including fixed pilot rows and deterministic draw generation, and then concatenated to 310 total sample rows.
- Built dedicated DuckDB-backed matchers (`xlsx_matcher`, `csv_matcher`, `docx_matcher`, `parquet_matcher`) that append `InnerDict` entries with `SourceKey` provenance.
- Updated configuration, dependencies, and pixi tasks to reference the new module layout.
