...
CODEX_EXEC_COMMAND: Final = ("codex", "exec", "--skip-git-repo-check", "-")
...
HISTORY_CELL_RENDERER: Final = """params => {
    const rows = params.data.attempt_history || [];
    if (!rows.length) return '';
    const details = document.createElement('details');
    const summary = document.createElement('summary');
    summary.textContent = `${rows.length} earlier attempt${rows.length === 1 ? '' : 's'}`;
...
class PushAcceptedRequest(BaseModel):
    ...
    def normalized_nonblank(cls, value: str) -> str:
        if not value or value != value.strip():
            raise ValueError("accepted-run identifiers must be non-blank and normalized")
        return value
...
class RuntimeConfiguration:
    def __init__(
        ...
        *,
        ...
        try:
            eligible_cohorts = derive_eligible_cohorts(source_connection, release_batches)
        finally:
...
class SourceRepository:
    ...
    def load_eligible_researchers(self) -> tuple[Researcher, ...]:
        ...
        if cohorts is None:
            raise RuntimeError("eligible cohorts were not configured")
        connection = self.connect()
        try:
            researchers = []
            for source_key, cohort in sorted(cohorts.items()):
                source = load_source_researcher(
                    connection,
                ...
                researchers.append(Researcher(
                    ...
                    draw_numbers=source.draw_numbers,
                    first_name=source.first_name,
                    last_name=source.last_name,
                    cohort=ResearcherCohort(cohort),
                ))
            result = tuple(researchers)
        finally:
            connection.close()
    ...
    def load_ground_truth_by_source_key(
        ...
        try:
            for source_key, cohort in sorted(cohorts.items()):
                ...
                values = ground_truth_for_researcher(researcher)
                if values is None:
                    raise RuntimeError("ground-truth cohort contains no ground truth")
    ...
    def assert_population_invariants(
        ...
        if len(set(source_keys)) != len(source_keys):
            raise RuntimeError("eligible researcher source keys are not unique")
        if (
            ground_truth_count,
            ...
            raise RuntimeError("eligible researcher population invariants failed")
    ...
    def load_accepted_attempts(
        ...
        try:
            ...
            rows = connection.execute(
                ...
                f"ORDER BY {duckdb_quote_identifier(KTP_SOURCE_KEY_COL)}, "
        ...
        for row in rows:
            ...
            source_key = SourceKey(str(values[KTP_SOURCE_KEY_COL]))
            try:
                metadata_value = json.loads(str(values[KTP_AI_AUGMENT_SESSION_METADATA_COL]))
                metadata = SessionMetadata(
                    originator=str(metadata_value["originator"]),
                    source=str(metadata_value["source"]),
                    cli_version=str(metadata_value["cli_version"]),
                    model_provider=str(metadata_value["model_provider"]),
                    model=str(metadata_value["model"]),
                    reasoning_effort=str(metadata_value["reasoning_effort"]),
                    session_id=SessionId(str(metadata_value["session_id"])),
                    timestamp=datetime.fromisoformat(
                        str(metadata_value["timestamp"]).replace("Z", "+00:00")
                    ),
                )
            except (KeyError, TypeError, ValueError, json.JSONDecodeError) as exc:
                raise RuntimeError("accepted output has invalid session metadata") from exc
    ...
    def append(
        ...
        try:
            if os.write(descriptor, payload) != len(payload):
                raise OSError("run journal append was incomplete")
            os.fsync(descriptor)
        finally:
            os.close(descriptor)
        ...
        with self._path.open(encoding="utf-8") as stream:
            for line_number, line in enumerate(stream, start=1):
                try:
                    ...
                    raise RuntimeError(
                        f"run journal is malformed at line {line_number}"
                    ) from exc
        return tuple(events)
        ...
        for event in self.load_events():
            if event.kind is RunEventKind.QUEUED:
                if event.run_id in runs:
                    raise RuntimeError("run journal contains a duplicate queued run ID")
            ...
            if run is None or run.source_key != event.source_key:
                raise RuntimeError("run journal event has no matching queued run")
            if event.kind is RunEventKind.STARTED:
                ...
                if event.session_id is None:
                    raise RuntimeError("session-discovered event has no session ID")
                ...
                if event.rollout_jsonl is None:
                    raise RuntimeError("rollout-discovered event has no path")
                ...
                if event.accepted_attempt_id is None:
                    raise RuntimeError("push-accepted event has no attempt ID")
...
class ResearcherCardRenderer:
    ...
    def render(
        ...
        researchers = {
            ...
            for researcher in self._source_repository.load_eligible_researchers()
        }
        researcher = researchers.get(source_key)
        if researcher is None:
            raise KeyError(f"unknown eligible source key: {source_key}")
        cards = build_cards(
            ...
            intro=CARD_INTRODUCTION.format(
                datetime.now(self._configuration.timezone).strftime("%B %d, %Y")
            ),
            excluded_cols=CARD_EXCLUDED_COLUMNS,
        )
        if len(cards) != 1:
            raise RuntimeError("selected researcher did not render exactly one card")
        ...
        def request_openapi() -> None:
            ...
            with urllib_request.urlopen(
                ...
                if response.status != status.HTTP_200_OK:
                    raise RuntimeError("backend OpenAPI endpoint is not ready")
...
class CodexRunner:
    ...
    def codex_remote_command(
        ...
        *,
        ...
        return (
            ...
            f"exec {shlex.join(CODEX_EXEC_COMMAND)} "
            f"< {shlex.quote(str(CODEX_PROMPT_PATH))}"
        )
    ...
    async def _remote_command(
        ...
        *,
        ...
        if check and process.returncode != 0:
            raise RuntimeError(
                f"AIVM command failed with exit {process.returncode}: "
    ...
    async def start(
        ...
        *,
        ...
        await self._remote_command(
            f"mkdir -p -- {shlex.quote(str(CODEX_WORKDIR))} && "
    ...
    async def discover_session(
        ...
        find_command = (
            f"find {shlex.quote(str(CODEX_SESSIONS_ROOT))} -type f "
            f"-name 'rollout-*.jsonl' -newer {shlex.quote(str(marker_path))} -print | "
            "LC_ALL=C sort | tail -n 1"
        )
        while loop.time() < deadline:
            pid_text = (
                await self._remote_command(
                    f"cat -- {shlex.quote(str(pid_path))}",
                    check=False,
                )
            ).decode("utf-8").strip()
            if pid_text.isdecimal():
                handle.remote_pid = RemotePid(int(pid_text))
            rollout_text = (await self._remote_command(find_command)).decode("utf-8").strip()
            if rollout_text:
                rollout_path = PurePosixPath(rollout_text)
                first_line = (
                    await self._remote_command(
                        f"head -n 1 -- {shlex.quote(str(rollout_path))}"
                    )
                ).decode("utf-8")
                try:
            ...
            if handle.process.returncode is not None:
                raise RuntimeError("Codex exited before its session was discovered")
            await asyncio.sleep(CODEX_DISCOVERY_POLL_SECONDS)
        raise TimeoutError("Codex session was not discovered")
    ...
    async def discover_rollout_path(
        ...
        *,
        ...
        output = await self._remote_command(
            f"find {shlex.quote(str(CODEX_SESSIONS_ROOT))} -type f "
            f"-name {shlex.quote(rollout_name)} -print | LC_ALL=C sort"
        )
        paths = [PurePosixPath(line) for line in output.decode("utf-8").splitlines() if line]
        if len(paths) != 1:
            raise RuntimeError("Codex rollout path did not resolve uniquely")
        return paths[0]
    ...
    async def cancel(
        ...
        if handle.remote_pid is not None:
            await self.terminate_remote_pid(handle.remote_pid)
    ...
    async def terminate_remote_pid(
        ...
        pid = int(remote_pid)
        if pid <= 0:
            raise ValueError("remote PID must be positive")
        await self._remote_command(f"kill -TERM -- {pid}", check=False)
    ...
    async def sanction(
        ...
        async with self._lock:
            if self._sanctioned_run is not None:
                raise RuntimeError("another run is already sanctioned")
...
class VariableProjector:
    @staticmethod
    def action_for_status(status: RunStatus) -> RunAction:
    ...
    def project_attempt(
        ...
        *,
        ...
        return AttemptVariableProjection(
            ...
            action=self.action_for_status(attempt.status),
        )
...
class ControlCentreController:
    ...
    async def start(self) -> None:
        self._researchers = await asyncio.to_thread(
            self._source_repository.load_eligible_researchers
        )
        ...
        for run in tuple(self._runs.values()):
            if run.status in {RunStatus.QUEUED, RunStatus.RUNNING}:
                self._append_run_event(RunEvent(
                    ...
                    detail="Control Centre restarted before the run completed",
                ))
    ...
    async def queue(
        ...
        *,
        ...
        if source_key not in self._researchers_by_source_key:
            raise KeyError(f"unknown eligible source key: {source_key}")
        run_id = uuid4()
        self._append_run_event(RunEvent(
            run_id=run_id,
    ...
    async def cancel(
        ...
        *,
        ...
        if self._active_run_id == run_id and self._active_codex is not None:
            await self._codex.cancel(self._active_codex)
    ...
    async def acknowledge_push(
        ...
        *,
        ...
        if (
            ...
            raise ValueError("accepted push does not match the current sanctioned run")
        self._append_run_event(RunEvent(
            run_id=run_id,
    ...
    async def snapshot(
        ...
        *,
        ...
        statuses = [view.current_status for view in views]
        counts = DashboardCounts(
            total=len(views),
            ground_truth=sum(
        ...
        card = None
        if selection.selected_source_key is not None and self._active_run_id is None:
            card = await self.researcher_card(source_key=selection.selected_source_key)
        return UiSnapshot(
            counts=counts,
            rows=rows,
            card=card,
    ...
    async def researcher_card(
        ...
        *,
        ...
        if self._active_run_id is not None:
            raise RuntimeError("detour card reads are suspended while a run is active")
    ...
    async def _execute_run(
        ...
        *,
        ...
        run = self._runs[run_id]
        result = await self._codex.start(run_id=run_id)
        ...
        self._append_run_event(RunEvent(
            ...
            remote_pid=(
                None if result.handle.remote_pid is None else int(result.handle.remote_pid)
            ),
        ))
        self._append_run_event(RunEvent(
            run_id=run_id,
    ...
    async def _accepted_attempt_for_session(
        ...
        *,
        ...
        if len(matches) > 1:
            raise RuntimeError("accepted output contains duplicate session attempts")
...
class ControlCentrePage:
    ...
    def build_header(self) -> None:
        with ui.row():
            ui.label("AI augmentation Control Centre")
            self._handles.backend_status_label = ui.label("Backend: starting")

    def build_summary(self) -> None:
        self._handles.summary_label = ui.label("Loading researcher state…")
    ...
    def build_filters(self) -> None:
        with ui.row():
            ...
            self._handles.status_select = ui.select(
                {"": "All statuses", **{status.value: status.value for status in RunStatus}},
                value="",
                label="Status",
            ...
            self._handles.search_input = ui.input(
                label="Search name, draw, or source key",
    ...
    def build_grid(self) -> None:
        ...
        self._handles.grid = ui.aggrid({
            "columnDefs": self.grid_column_definitions(variable=variable),
            "rowData": [],
            "defaultColDef": {"resizable": True, "sortable": True, "filter": True},
            "getRowId": ":params => params.data.source_key",
            "domLayout": "autoHeight",
        })
        self._handles.grid.on("cellClicked", self._on_grid_cell_clicked)

    def build_card_panel(self) -> None:
        self._handles.selected_researcher_label = ui.label("No researcher selected")
        self._handles.card_container = ui.card()
        with self._handles.card_container:
            self._handles.card_markdown = ui.markdown("")
    ...
    def grid_column_definitions(
        ...
        *,
        ...
        return [
            {"field": "draw_number", "headerName": "Draw(s)", "width": 110},
            {"field": "last_name", "headerName": "Last name", "width": 150},
            {"field": "first_name", "headerName": "First name", "width": 150},
            {"field": "cohort", "headerName": "Cohort", "width": 150},
            {
                "field": "ai_value",
                "headerName": variable.ai_column,
                "wrapText": True,
                "autoHeight": True,
                "flex": 2,
            },
            {
                "field": "table_1_value",
                "headerName": variable.table_1_column,
                "wrapText": True,
                "autoHeight": True,
                "flex": 2,
            },
            ...
            {
                "field": "footnote_arguments",
                "headerName": "Arguments",
                "wrapText": True,
                "autoHeight": True,
                "flex": 2,
            },
            {"field": "attempt_id", "headerName": "Attempt", "width": 190},
            {"field": "attempt_timestamp", "headerName": "Time", "width": 180},
            {"field": "status", "headerName": "Status", "width": 110},
            {
                "field": "history",
                "headerName": "Earlier attempts",
                "wrapText": True,
                "autoHeight": True,
                "flex": 2,
                ":cellRenderer": HISTORY_CELL_RENDERER,
            },
    ...
    def grid_options(
        ...
        *,
        ...
        return {
            "columnDefs": self.grid_column_definitions(variable=variable),
            "rowData": self.grid_rows(snapshot=snapshot),
            "defaultColDef": {"resizable": True, "sortable": True, "filter": True},
            "getRowId": ":params => params.data.source_key",
            "domLayout": "autoHeight",
        }
        ...
        for row in snapshot.rows:
            ...
            for attempt in older_attempts:
                ...
                history_lines.append(
                    f"{timestamp} | {attempt.attempt_status.value} | "
                    f"{attempt.attempt_id or ''} | {attempt.ai_value or ''}"
                )
            rows.append({
                "source_key": row.source_key,
                "run_id": None if latest.run_id is None else str(latest.run_id),
                "draw_number": latest.draw_number,
                "last_name": latest.last_name,
                "first_name": latest.first_name,
                "cohort": row.cohort.value,
                "ai_value": latest.ai_value,
                "table_1_value": latest.table_1_value,
                "footnotes": latest.footnotes,
                "footnote_arguments": latest.footnote_arguments,
                ...
                "status": latest.attempt_status.value,
                "history": "\n".join(history_lines),
                "attempt_history": self.attempt_detail_rows(row=row)[:-1],
        ...
        return [
            {
                ...
                "status": attempt.attempt_status.value,
                "ai_value": attempt.ai_value,
    ...
    async def refresh(self) -> None:
        ...
        if self._handles.backend_status_label is not None:
            self._handles.backend_status_label.set_text(
                f"Backend: {snapshot.backend_status.value}"
            )
        if self._handles.summary_label is not None:
            counts = snapshot.counts
            self._handles.summary_label.set_text(
                f"Total {counts.total} · ground truth {counts.ground_truth} · "
                f"no ground truth {counts.no_ground_truth} · ready {counts.ready} · "
        ...
        if snapshot.card is not None:
            await self._show_card(snapshot.card)
    ...
    async def refresh_grid(self) -> None:
        ...
        snapshot = await self._controller.snapshot(selection=self._selection)
        variable = VARIABLE_SPEC_BY_KEY[self._selection.variable_key]
        self._handles.grid.options = self.grid_options(
            snapshot=snapshot,
            variable=variable,
        )
    ...
    async def _show_card(self, card: ResearcherCardView) -> None:
        if self._handles.selected_researcher_label is not None:
            self._handles.selected_researcher_label.set_text(
                f"{card.last_name}, {card.first_name} · draw(s) {card.draw_number}"
            )
    ...
    async def on_variable_changed(
        ...
        if variable_key not in VARIABLE_SPEC_BY_KEY:
            raise KeyError(f"unknown variable: {variable_key}")
    ...
    async def on_queue(
        ...
        await self._controller.queue(source_key=source_key)
        await self.refresh_grid()
    ...
    async def on_rerun(
        ...
        await self._controller.rerun(source_key=source_key)
        await self.refresh_grid()
    ...
    async def _on_grid_cell_clicked(self, event: Any) -> None:
        ...
        source_key = SourceKey(str(data.get("source_key", "")))
        if not source_key:
            return
        if arguments.get("colDef", {}).get("field") == "action":
            run_id_value = data.get("run_id")
            await self.on_grid_action(
                action=RunAction(str(data["action"])),
                source_key=source_key,
                run_id=None if run_id_value is None else UUID(str(run_id_value)),
            )
        else:
            await self.on_researcher_selected(source_key)
...
def require_services() -> ApplicationServices:
    if SERVICES is None:
        raise RuntimeError("Control Centre services have not started")
    return SERVICES
