...
1. At provisioning, the Human Operator issues one host-held SSH (Secure Shell) protocol credential at `AIVM_KEY_DIR` for own use downstream to connect to the AI Agent Runtime. Its public key is authorized on the guest machine separately for the ordinary account used by the AI Agent Runtime and for the restricted read account used by the Backend API to snapshot the `appendwatch` report and Codex session rollouts.
