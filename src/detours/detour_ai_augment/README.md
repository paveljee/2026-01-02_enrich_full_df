...
1. Given it is a queued namekey's turn to run, and given the AI Agent Runtime is up, and given the Backend API is down, and given no Codex processes are running on the guest machine on which AI Agent Runtime is running, the dashboard module of the Control Centre configures and starts the Backend API. Then the dashboard module of the Control Centre starts a non-interactive Codex session within the running AI Agent Runtime as described below. The dashboard module of the Control Centre then monitors the Codex process it started. When the Codex process exits, the dashboard module of the Control Centre queries the detour DB through the Backend API-owned and exposed Unix socket; if it finds an accepted attempt for that Codex session, it records the run as complete; else it records the run as failed. A Human Operator cancellation of the current run, initiated through the GUI of the dashboard module of the Control Centre at any time, marks the run as cancelled. A run marked as complete, failed, or cancelled initiates the immediate winding down of the Backend API and the Codex processes the dashboard module of the Control Centre started, if any, and any child processes. A Human Operator cancellation of a queued run whose turn to run has not come yet simply removes the queued run from the queue maintained by the dashboard module of the Control Centre.
