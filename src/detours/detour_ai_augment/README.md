...
1. Given it is a queued namekey's turn to run, and given the Backend API and the AI Agent Runtime are down, the dashboard module of the Control Centre configures and starts the Backend API and the AI Agent Runtime as described above. The dashboard module of the Control Centre then monitors the non-interactive Codex process that it started as part of the AI Agent Runtime startup. When the Codex process exits, the dashboard module of the Control Centre queries the detour DB through the Backend API-owned and exposed Unix socket; if it finds an accepted attempt for that Codex session, it records the run as complete; else it records the run as failed. A Human Operator cancellation of the current run, initiated through the GUI of the dashboard module of the Control Centre at any time, marks the run as cancelled and initiates the immediate winding down of the Backend API and AI Agent Runtime. A Human Operator cancellation of a queued run whose turn to run has not come yet, it is simply removed from the queue maintained by the dashboard module of the Control Centre.
1. Given the dashboard module of the Control Centre started the Backend API or the AI Agent Runtime, and on any expected or unexpected exit of the dashboard module of the Control Centre, the dashboard module of the Control Centre graciously winds down the Backend API and AI Agent Runtime processes it started and any child processes.
1. Given the AI Agent Runtime is provisioned and has started, and given the Backend API has started and its `GET /openapi.json` responds `200 OK`, the Human Operator connects to the AI Agent Runtime using the SSH credentials issued above and initializes a new Codex session.
