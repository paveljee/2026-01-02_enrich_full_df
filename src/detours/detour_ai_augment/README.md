...
1. Given the `APPENDWATCH_REPORT` has been read, the Backend API prepares but does not send `POST http://invalid/commit`. The request has an empty query and null response and is represented by this exact `HttpRequestLogRecord(schema_version="1.1")` contour: `{"schema_version":"1.1","record_id":"<commit UUIDv7>","method":"POST","scheme":"http","host":"invalid","port":null,"coerce_schema_v1":false,"ready_to_respond_at_unix_usec":null,"path":"/commit","query":"","request_headers":{"Source-Key":"ktp.filename=\"<rollout filename>\", ktp.fragment=<line count>, ktp.fragment_type=\"line_number\"","Name-Key":"ktp.first_name=\"<first name>\", ktp.last_name=\"<last name>\""},"request_body":{"schema_version":1,"pull_record_id":"<pull UUID>","push_record_id":"<push UUID>","rollout":{"sha256":"<SHA-256>","size":<byte size>,"line_count":<line count>},"appendwatch_report":{"encoding":"base64","data":"<base64 report bytes>"}},"response_code":null,"response_headers":{},"response_body":null,"received_at_unix_usec":null,"duration_usec":0}`.
...
1. The validation function resolves the committed pull and push from the detour DuckDB database, verifies the rollout SHA-256, byte size, and line count, and parses the rollout filename and Codex session ID. It requires the committed `Source-Key` to match the rollout and the committed `Name-Key` to match the referenced pull.
1. The validation function decodes the committed appendwatch report, requires exactly one report leaf whose filename equals the committed `Source-Key`'s `ktp.filename`, and rejects a missing or duplicate leaf, a `COMPROMISED` leaf, or a `COMPROMISED` ancestor. The report itself is the durable appendwatch result; no separate appendwatch response is persisted.
1. Given appendwatch validation passed, the validation function selects `Submission` for a push referring to the original NDJSON pull and `StandardizedSubmission` for a push referring to a Markdown follow-up pull, then validates the committed push against the selected Pydantic model, rollout, and evidence rules.
1. Validation ends by making its result available at `GET /pull`. An expected submission or evidence validation failure makes resubmission instructions available as `200 OK` with `Content-Type: text/markdown; charset=utf-8`; this pull becomes the pull referenced by the next push. An integrity, configuration, or unexpected failure makes an opaque `500 Internal Server Error` available, instructing the AI Agent Runtime to contact the Human Operator. These outcomes do not alter the preceding `202 Accepted` push response.
1. Successful validation writes the final innerdict into the detour DuckDB database for the Codex session and makes `410 Gone` available at `GET /pull` with `Content-Type: application/x-ndjson; charset=utf-8`. The first response line is the accepted innerdict; the optional second line is ground truth. When replayed, the persisted pull response reconstructs the final innerdict and links it to the pull, push, commit, rollout, and appendwatch report.
...
- The detour features a replayable JSON Lines log of all public `/pull` and `/push` exchanges and private `/commit` requests. One shared function validates, appends, and `fsync`s every record before returning a public response or invoking validation.
- Codex rollout files are immutable content-addressed files. The detour DuckDB database is a faithful projection of the replay log and referenced rollout files, synchronized before use.
- The Control Centre features a dashboard UI that facilitates queueing of HCR profiles. Queueing is maintained in NiceGUI's `app.storage.general` and never enters the replay log or detour DuckDB database.
