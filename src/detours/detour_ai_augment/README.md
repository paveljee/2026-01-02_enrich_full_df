...
1. At provisioning, the Human Operator connects to the AI Agent Runtime using the SSH credentials and ensures that `OPENALEX_API_KEY` is exported as an environment variable on the guest machine on which the AI Agent Runtime is running. This guest key is provided exclusively for availability to the AI Agent Runtime; the Backend API reads the host `OPENALEX_API_KEY` from the repository `.env` instead (as such, do not assert equality of the guest and host `OPENALEX_API_KEY`).
...
1. Given it is a queued namekey's turn to run, and given the Backend API was down, the dashboard module of the Control Centre configures and starts the Backend API as described above.
1. Given the dashboard module of the Control Centre started the Backend API, and on any expected or unexpected exit of the dashboard module of the Control Centre, the dashboard module of the Control Centre graciously winds down the Backend API process it started and any child processes.
